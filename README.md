<h1 align="center">Hi 👋, I'm Hoang Duc Kien</h1>
<h3 align="center">Data Analyst</h3>

---

## 🚀 About Me

I graduated in Information Systems from VNU-HCM University of Science and have a strong foundation in data analytics and databases. Through projects in SQL, Python, and Power BI, I developed analytical thinking and clear data storytelling. I am eager to begin my Data Analyst career, solve real business problems, and keep improving through hands-on experience.

---

## 📊 Featured Projects

### 🛒 E-commerce Performance & Conversion Analysis (SQL, BigQuery)
- Analyzed 1M+ GA records and built a full conversion funnel.
- Evaluated traffic quality, revenue trends, and user behavior.

🔗 View Project

---

### 🔁 Customer Churn Prediction & Segmentation (Python, Machine Learning)
- Built churn prediction models and customer clusters.
- Proposed retention strategies for high-risk segments.

🔗 View Project

---

### 📈 Sales Performance & Market Analysis (Power BI)
- Built interactive Power BI dashboards from 50K+ sales records.
- Highlighted profitability, regional performance, and product trends.

---

### 🎯 RFM Customer Segmentation (Python)
- Performed RFM segmentation on 500K+ transactions.
- Identified high-value and at-risk customer groups.


# GitHub Stats:
![](https://github-readme-stats.vercel.app/api?username=duckien2012&theme=radical&hide_border=true&include_all_commits=true&count_private=true)<br/>
![](https://github-readme-streak-stats.herokuapp.com/?user=duckien2012&theme=radical&hide_border=true)<br/>
